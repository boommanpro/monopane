...
export class CopyShortcut implements ShortcutsHandler {
  ...
  private getValidNodes(nodes: WorkflowNodeEntity[]): WorkflowNodeEntity[] {
    return nodes.filter((n) => {
      if (
        [CanvasNodeType.FlowStart, CanvasNodeType.FlowEnd].includes(
          n.flowNodeType as CanvasNodeType
        )
      ) {
        return false;
      }
  ...
  private notifySuccess(): void {
    const startEndNodeTypes: CanvasNodeType[] = [CanvasNodeType.FlowStart, CanvasNodeType.FlowEnd];
    if (
      this.selectedNodes.some((node) =>
        startEndNodeTypes.includes(node.flowNodeType as CanvasNodeType)
      )
    ) {
      Toast.warning({
        content: '流程起点 / 终点节点不可复制，其余节点已复制到剪贴板',
        showClose: false,
      });
      return;
    }
